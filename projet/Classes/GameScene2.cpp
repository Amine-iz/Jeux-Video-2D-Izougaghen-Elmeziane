...
bool GameScene2::init()
{
    ...
    auto backgroundSprite = Sprite::create("terre2.png");
    backgroundSprite->setPosition(Point(visibleSize.width / 2 + origin.x, visibleSize.height / 2 + origin.y));
    this->addChild(backgroundSprite);

    auto edgebody = PhysicsBody::createEdgeBox(visibleSize, PHYSICSBODY_MATERIAL_DEFAULT, 3);
    auto edgeNode = Node::create();
    edgeNode->setPosition(Point(visibleSize.width / 2 + origin.x, (visibleSize.height / 2) + 195 + origin.y));
    edgeNode->setPhysicsBody(edgebody);
    this->addChild(edgeNode);



    // auto action = MoveBy::create(20, Point(100, 0));
     //sprite->runAction(action);

    void addObstacle1(float dt); {
        auto addObstacle1 = Sprite::create("obstacle1.png");
        addObstacle1->setPosition(Point(visibleSize.width / 2 +10, visibleSize.height / 2 + origin.y));
        auto spriteBd = PhysicsBody::createBox(addObstacle1->getContentSize(), PhysicsMaterial(1, 1, 1));
    ...
    void addObstacle2(float dt); {
        auto addObstacle2 = Sprite::create("obstacle3.png");
        addObstacle2->setPosition(Point(visibleSize.width, visibleSize.height / 2 + origin.y));
     auto spriteBd = PhysicsBody::createBox(addObstacle2->getContentSize(), PhysicsMaterial(1, 1, 1));
     addObstacle2->setPhysicsBody(spriteBd);
        this->addChild(addObstacle2); }


    void addObstacle3(float dt); {
        auto addObstacle3 = Sprite::create("obstacle1.png");
        addObstacle3->setPosition(Point((visibleSize.width / 4) - 40 , visibleSize.height / 2 + origin.y));
    ...
    auto playItem = MenuItemFont::create("Menu", CC_CALLBACK_1(GameScene2::GoToMenuScene, this));
...
void GameScene2::GoToMenuScene(cocos2d::Ref* sender)
{
